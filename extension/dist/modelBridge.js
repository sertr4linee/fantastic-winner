...
function logStreamChunk(chunkIndex, chunk) {
    logToFile(`CHUNK #${chunkIndex}: ${JSON.stringify(chunk)}`);
}
function logStreamComplete(fullResponse, totalChunks) {
    logToFile('─'.repeat(60));
...
class ModelBridge {
     ...
     */
    async sendToCopilotChat(prompt, onWord, onComplete, onError, onChatOpened) {
        try {
